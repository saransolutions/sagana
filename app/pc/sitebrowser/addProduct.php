...
if(isset($_GET['operation'])) {
	//$conn = db::get('mysqli://saganaj@127.0.0.1/saganaje_db');
	//$mysqli = new mysqli("127.0.0.1", "saganaje_user", "saganaje_pass", "saganaje_db", 3306);
	$fs = new tree(db::get('mysqli://saganaje_user:saganaje_pass@127.0.0.1/saganaje_db'), array('structure_table' => 'tree_struct', 'data_table' => 'tree_data', 'data' => array('nm')));
	try {
		$rslt = null;
		switch($_GET['operation']) {
			...
			case "get_content":
				...
				if(count($node) > 1) {
					...
					$rslt = array('content' => 'choosen: /' . implode('/',array_map(function ($v) { return $v['nm']; }, $temp['path'])). '/'.$temp['nm']);					
				}
				break;
			case 'create_node':
				$node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int)$_GET['id'] : 0;
	...
	echo "the selected value is - ".$_GET['content'];
}
?>
<!DOCTYPE html>
<html>
	<head>
		...
		<title>Sagana Jwellery - Add Product</title>
	...
	<div id='container1'>
	<h2><center>Sagana Jwellery - Product Catalog</center></h2>
		<div id='navigation' >
    	<ul>		  
...
<div id="container" role="main" style='background:none;'>
			<div id="tree"></div>
			<div id="data">
